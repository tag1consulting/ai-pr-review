...
| Bitbucket Cloud | `bitbucket` | ✅ | ✅ (via Code Insights annotations, not inline comments -- see [Bitbucket setup](docs/bitbucket-setup.md)) | ❌ | ✅ (real approve/request-changes calls, default on via `AI_BITBUCKET_REVIEW_STATE`) |
| GitLab | `gitlab` | ✅ | ✅ | ✅ | ✅ |
...
| `AI_REVIEW_FEEDBACK_LOOP` | Variable | No | `true` to enable the learning loop (GitHub + Bitbucket; default `false`) |
...
| `feedback-loop` | No | `false` | Persist `/ai-pr-review false-positive\|wont-fix\|feedback` verdicts to a dedicated git branch and re-inject them into future reviews. GitHub and Bitbucket (Bitbucket: `false-positive`/`wont-fix` only; `feedback` command support is tracked separately in issue #933). |
