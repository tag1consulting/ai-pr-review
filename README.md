...
| `AI_REVIEW_IMAGE_TAG` | Variable | No | Container image tag (default `latest`; set to `dev` to dogfood pre-release builds or pin to a release like `0.11.0` — image tags published by `publish-image.yml` strip the `v` prefix) |
