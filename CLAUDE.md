...
| `ai_pr_review/vcs/_canonical.py` | Canonical-review reuse (GitHub only, #710 tracks GitLab/Bitbucket parity): `select_canonical`/`merge_verdicts` pick the highest-id bot review and union its verdicts marker across every prior review (shared with `slash/dismiss.py`'s `_record_verdict`); `classify()` runs the per-finding decision table (suppress/recur/update/escalate/new) against verdicts + fuzzy `(file, ±3 lines, compatible category)` thread matches; `decide_action()` decides `put` (reuse the canonical body) vs `post` (fresh review) for `github.py:post_findings` |
