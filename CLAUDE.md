...
| `ai_pr_review/vcs/_canonical.py` | Canonical-review reuse (GitHub only, #710 tracks GitLab/Bitbucket parity): `select_canonical`/`merge_verdicts` pick the highest-id bot review with a non-empty body (skipping GitHub's auto-created empty-body reply reviews) and union its verdicts marker across every prior review (shared with `slash/github_orchestration.py`'s `_record_verdict`); `classify()` runs the per-finding decision table (suppress/recur/update/escalate/new) against verdicts + fuzzy `(file, ±3 lines, compatible category)` thread matches; `decide_action()` decides `put` (reuse the canonical body) vs `post` (fresh review) for `github.py:post_findings` |
...
| `ai_pr_review/vcs/_thread.py` | (#822) `first_comment*` GraphQL review-thread accessors (previously duplicated between `github.py` and `slash/github_orchestration.py`) and `count_unresolved_owned_threads` (the shared "unresolved threads per owning review" counter behind `github.py`'s `_dismiss_stale_reviews` and `slash/github_orchestration.py`'s `_dismiss_if_all_resolved`/`_approve_if_pr_fully_resolved`) — callers still supply their own `bot_login` (`None` in `github_orchestration.py`, a normalized `graphql_bot_login(...)` in `github.py`), an intentional difference, not a bug |
