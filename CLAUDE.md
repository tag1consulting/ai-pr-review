...
| `ai_pr_review/findings/provenance.py` | Provenance-weighting: computes corroboration signal — sets `Finding.corroborated = True` when the same finding is independently confirmed by both an LLM agent and a static analyzer; called during the merge phase |
