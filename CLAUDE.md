...
2. Add an `AgentSpec` entry to `ai_pr_review/agents/roster.py` with the agent name, prompt path, tier (1 or 2 — controls parallel dispatch group), `max_output_tokens`, `full_mode_only` flag, `conditional_trigger` (file-pattern or `None`), and `context_enrichment_eligible` flag. `max_output_tokens` must fall within `AGENT_MAX_TOKENS_MIN`/`AGENT_MAX_TOKENS_MAX` (256–65536, same module). This value is also the agent's runtime default budget, overridable per-agent via `AI_MAX_TOKENS_<AGENT_UPPER_SNAKE>` (issue #191) with no separate registration needed — `resolve_agent_max_tokens()` in `config.py` derives the env var name from the roster `name` automatically. A `separately_dispatched=True` agent (bypassing the generic tier-dispatch loop, like `pr-summarizer`/`issue-linker`) must instead look its own default up via `get_agent(name).max_output_tokens` at its call site rather than hand-typing a second copy of the number — issue #847 fixed exactly that drift for the two existing separately-dispatched agents.
