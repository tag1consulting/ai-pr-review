...
| `ai_pr_review/review/runtime.py` | `build_review_runtime`: assembles `ReviewRuntime` (diff context, manifest, language profiles, agent roster, dispatch context, SARIF extras) consumed by `cli`. Also runs static analyzers/SARIF, then the pre-flight cost-ceiling check (`review/cost_ceiling.py`), in that order (#848) — a `CostCeilingExceeded` skip must not suppress the free analyzer/SARIF pass that already ran |
| `ai_pr_review/review/cost_ceiling.py` | Pre-flight LLM-cost estimate + ceiling (#24, closes issue #24; follow-up debt in #848): `estimate_review_cost`/`estimate_preflight_agent_cost` estimate cost from diff size, agent roster, and `pricing.py` rates (a deliberately biased-high approximation — full output cap per agent, no caching discount, no judge-pass call; see module docstring); `enforce_cost_ceiling` raises `CostCeilingExceeded` when `AI_MAX_COST_USD` is exceeded, which `review/runtime.py` turns into a `SkipPlan` before any LLM call; `merge_cost_estimates` combines the main roster's `CostEstimate` with individual `AgentCostEstimate`s (e.g. pr-summarizer/issue-linker) |
| `ai_pr_review/review/preflight.py` | Pre-review LLM agents: `run_summarizer` (pr-summarizer) and `run_issue_linker` (issue-linker); `fetch_open_issues` gh-CLI helper; all fail-soft. `should_run_pr_summarizer`/`should_run_issue_linker` and `PREFLIGHT_AGENT_MAX_TOKENS` (#848) are the single source of truth for "will this agent dispatch this run", shared with `cost_ceiling.py`'s pre-flight estimate so the two gates can't drift |
...
| `ai_pr_review/vcs/_canonical.py` | Canonical-review reuse (GitHub only, #710 tracks GitLab/Bitbucket parity): `select_canonical`/`merge_verdicts` pick the highest-id bot review with a non-empty body (skipping GitHub's auto-created empty-body reply reviews) and union its verdicts marker across every prior review (shared with `slash/dismiss.py`'s `_record_verdict`); `classify()` runs the per-finding decision table (suppress/recur/update/escalate/new) against verdicts + fuzzy `(file, ±3 lines, compatible category)` thread matches; `decide_action()` decides `put` (reuse the canonical body) vs `post` (fresh review) for `github.py:post_findings` |
| `ai_pr_review/vcs/_{body,inline,finding_ids,stale}.py`, `marker.py`, `protocol.py`, `http.py` | Shared formatting / inline-eligibility / F-ID assignment / stale-thread helpers / `VcsProvider` protocol / retrying httpx client. `marker.py` also carries the per-inline-comment metadata marker (`ai-pr-review-finding`, base64-encoded fingerprint/category/severity) `_canonical.py`'s classification reads, and `USAGE_MARKER`/`build_usage_block` (#758) — the mode-independent anchor preceding the token-usage payload in every `token-usage-display` mode, shared by GitLab's summary-note upsert, Bitbucket's walkthrough-boundary extraction, and `_finding_ids.py`'s `_ends_body_section` (also used by `slash/dismiss.py`) |
...
| `ai_pr_review/vcs/_thread.py` | (#822) `first_comment*` GraphQL review-thread accessors (previously duplicated between `github.py` and `slash/dismiss.py`) and `count_unresolved_owned_threads` (the shared "unresolved threads per owning review" counter behind `github.py`'s `_dismiss_stale_reviews` and `slash/dismiss.py`'s `_dismiss_if_all_resolved`/`_approve_if_pr_fully_resolved`) — callers still supply their own `bot_login` (`None` in `dismiss.py`, a normalized `graphql_bot_login(...)` in `github.py`), an intentional difference, not a bug |
