...
| `analyzers/run-checkov.sh` | Wraps checkov for changed IaC files (`.tf`, `.tfvars`, `Dockerfile*`; also `.yaml`/`.yml` containing `apiVersion:`+`kind:` or `AWSTemplateFormatVersion:`; also `.json` containing `AWSTemplateFormatVersion`); all findings→Medium (no per-check severity from checkov); confidence 80; source `"checkov"` |
