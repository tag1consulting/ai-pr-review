...
- **`Finding.file` must be repo-relative, matching the diff and the `changed_files` manifest — never absolute, and never relative to the analyzer subprocess's cwd if that differs from the repo root.** Some wrapped tools resolve reported paths to absolute regardless of how the file was named on the command line — verified directly for ruff (see `_paths.py`'s own module docstring); phpcs, phpstan, and eslint are consolidated onto the same helper on the same assumption but not independently re-verified per tool. If yours does this, strip that prefix back off before constructing the `Finding` — see `ai_pr_review/analyzers/native/_paths.py::strip_workspace_prefix()` (tries `GITHUB_WORKSPACE`, then falls back to cwd on a mismatch or when unset) and use it rather than reimplementing prefix-stripping; note that `golangci_lint.py`, `docs_refs.py`, and `semgrep.py` still assume `cwd == repo root` via their own separate logic (prefix-prepending / relative-to, not stripping) and were not folded into this helper (#846 review, known-remaining). An analyzer that skips this silently loses severity: `findings/scope.py`'s `apply_diff_scope` can never match an absolute path against the diff's repo-relative line set, so the finding is always treated as out-of-diff (or dropped, under `AI_ANALYZER_DIFF_SCOPE=drop`) — it does log an aggregated warning when this happens, but the fix is not emitting the absolute path in the first place. This is the bug class behind issues #713 and #846.
