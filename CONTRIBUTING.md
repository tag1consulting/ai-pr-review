...
- **`Finding.file` must be repo-relative, matching the diff and the `changed_files` manifest — never absolute, and never relative to the analyzer subprocess's cwd if that differs from the repo root.** Some wrapped tools (ruff, phpcs, phpstan, eslint) always resolve reported paths to absolute regardless of how the file was named on the command line; if yours does, strip that prefix back off before constructing the `Finding` — see `ai_pr_review/analyzers/native/_paths.py::strip_workspace_prefix()` (GITHUB_WORKSPACE-aware, falls back to cwd) and use it rather than reimplementing prefix-stripping. An analyzer that skips this silently loses severity: `findings/scope.py`'s `apply_diff_scope` can never match an absolute path against the diff's repo-relative line set, so the finding is always treated as out-of-diff and capped to Low (it does log a warning when this happens, but the fix is not emitting the absolute path in the first place). This is the bug class behind issues #713 and #846.
