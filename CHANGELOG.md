...
- **This repo's own `ruff.sarif` (generated by `.github/workflows/ai-review.yml`'s `sarif-prep` job) could silently lose repo-relative paths on a self-hosted runner (issue #846)**. `analyzers/sarif.py`'s `_sanitize_sarif_path` stripped only a hardcoded `(home/)?runner/work/<owner>/<repo>/` pattern, which matches GitHub-hosted runners but not a self-hosted runner's own (arbitrary) checkout path. Now checks `GITHUB_WORKSPACE` first — the Actions runtime sets it to the real checkout root regardless of runner type — and falls back to the hardcoded regex only when that env var isn't set. This is a SARIF/host-side fix, deliberately separate from `_paths.py::strip_workspace_prefix()` above (that helper serves native-analyzer subprocess output, a different code path with different path shapes).
- **No pipeline-wide detection when an analyzer emits an absolute `Finding.file` (issue #846)**. `findings/scope.py`'s `apply_diff_scope` silently demoted such a finding to Low/`out_of_diff` with no signal beyond the severity drop — this is exactly how #713 hid until a human noticed it in a dogfood review. Added a warning log (`Finding.file` starting with `/`) so the next instance of this bug class surfaces in logs immediately instead of silently degrading a finding's visibility.

### Changed

- **Consolidated `eslint.py`/`phpcs.py`/`phpstan.py`'s own cwd-only path-stripping copies onto `_paths.py::strip_workspace_prefix()` (issue #846)**. Each independently re-implemented the same "strip a resolved-absolute prefix back to repo-relative" logic as `ruff.py`/`docs_comments.py` (issue #713), using only `os.getcwd()`/`Path.cwd()` — no `GITHUB_WORKSPACE` awareness, so any of the three could emit a wrong path on a self-hosted runner whose checkout root isn't the analyzer subprocess's cwd. No behavior change for the common case (GITHUB_WORKSPACE unset falls back to cwd, matching the old behavior exactly); self-hosted runners now get the same GITHUB_WORKSPACE-aware stripping the ruff-backed analyzers already had.
