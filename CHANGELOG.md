...
- **Per-agent token-budget follow-up debt from issue #191's own review (issue #847)**. `ai_pr_review/review/preflight.py`'s `pr-summarizer`/`issue-linker` preflight calls resolved their base `max_output_tokens` from a hand-typed literal instead of the agent roster; both call sites now look their default up via `get_agent(name).max_output_tokens`, and the two agent-name strings are now roster-derived constants (`PR_SUMMARIZER_AGENT_NAME`/`ISSUE_LINKER_AGENT_NAME` in `ai_pr_review/agents/roster.py`) rather than bare literals — including at the three remaining hand-typed call sites in `cli.py`/`dispatch.py` this pass's own review found. `resolve_agent_max_tokens()`'s docstring (and `docs/configuration.md`'s public description) claimed an out-of-range `AI_MAX_TOKENS_<AGENT>` override "falls back to default" — the code actually clamps it to whichever bound ([256, 65536]) it crossed and uses that clamped value; only a non-integer value actually falls back to the pre-override default. Both are now worded correctly. The `[256, 65536]` clamp bound was hardcoded independently in three places (`AgentSpec.__post_init__`, `ReviewConfig`'s `max_tokens_per_agent` field validator, and `resolve_agent_max_tokens()`); consolidated into `AGENT_MAX_TOKENS_MIN`/`AGENT_MAX_TOKENS_MAX` in `ai_pr_review/agents/roster.py`, referenced everywhere instead of repeated literals. `AI_MAX_TOKENS_<AGENT>` override values are now validated at `ReviewConfig.from_env()` (config-load time) for every agent, not only when that agent actually dispatches — previously a typo'd or out-of-range override for an excluded/gated-off agent was a silent dead value that could go unvalidated indefinitely. `CONTRIBUTING.md`/`CLAUDE.md`'s "adding a new agent" checklist, and `README.md`, never documented the per-agent override mechanism issue #191 introduced (`AI_MAX_TOKENS_<AGENT>`); both now cover it, alongside the existing full reference table in `docs/configuration.md`.
- **Follow-up review of the above (issue #847's own PR) caught a real behavior-change risk and two smaller bugs before merge.** The roster's `pr-summarizer` entry had carried `16384` since #430 while `preflight.py`'s hand-typed `4096` was, in practice, the *only* budget ever exercised at runtime (`pr-summarizer` is `separately_dispatched=True`, so nothing else ever reads its roster `max_output_tokens`). Fixing `preflight.py` to look the value up from the roster — the change described above — would therefore have silently quadrupled a live, cost-affecting LLM call with no e2e verification, on the one preflight agent whose token usage never reaches the token-usage table, and risked exceeding Bitbucket's 32,000-byte summary-body cap (a ~60KB output at 16384 tokens). Lowered the roster entry back to `4096` (the value production has always actually used) so the "look it up instead of hand-typing it" fix is genuinely zero-behavior-change; raising `pr-summarizer`'s real budget is a separate decision that needs its own live e2e verification against provider body-size caps. Separately: the new WARNING-vs-INFO log line for `AI_MAX_TOKENS_<AGENT>` overrides (`agents/dispatch.py`, `review/preflight.py`) mislabeled an override as "roster default" and logged it at the wrong level whenever the override's value happened to equal the base default, because it compared resolved values instead of checking whether the env var was actually set (`config.py`'s new `per_agent_max_tokens_override_is_set()`); and the config-load-time eager validator no longer calls `resolve_agent_max_tokens()` to build its warning, since that made it name a specific "using {default}" number that a tier-dispatched agent's real `AI_MAX_TOKENS_PER_AGENT` fallback could then contradict at dispatch time — it validates independently now, and iterates agent names in sorted order so multi-warning output is deterministic. Added a test asserting `docs/configuration.md`'s hand-maintained per-agent token table matches the roster, closing the same class of drift this issue was originally filed for.
