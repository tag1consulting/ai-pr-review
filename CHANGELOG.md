...
- **GitLab cross-run dedup — identity plumbing (issue #710, part 1 of 2)**: every newly-posted inline discussion now carries a per-finding metadata marker (fingerprint/category/severity), the same one GitHub's canonical-review reuse already writes. Nothing reads it back yet — this PR is pure plumbing, landed first so production GitLab discussions start carrying real fingerprints before the fuzzy-match dedup logic that consumes them ships. Also adds `parse_gitlab_prior_thread()` (pure, tested) to `ai_pr_review/vcs/_canonical.py`, and the `AI_GITLAB_CROSS_RUN_DEDUP` env var (default `true`, not yet read anywhere) as the eventual kill-switch for part 2.

### Fixed

- `GitLabProvider.post_findings`'s failure detection compared `inline_posted == 0` against `len(findings) > 0`, which would misclassify a run with zero inline candidates (everything routed to the body) as a failure. Now compares against `len(inline_candidates)` (what was actually attempted) and requires a real HTTP error, not just "nothing needed posting."
