...
- **This repo's own `ruff.sarif` (generated by `.github/workflows/ai-review.yml`'s `sarif-prep` job) could silently lose repo-relative paths on a self-hosted runner (issue #846)**. `analyzers/sarif.py`'s `_sanitize_sarif_path` stripped only a hardcoded `(home/)?runner/work/<owner>/<repo>/` pattern, which matches GitHub-hosted runners but not a self-hosted runner's own (arbitrary) checkout path. Now tries `AI_SARIF_HOST_WORKSPACE` (new; see below), then `GITHUB_WORKSPACE`, before falling back to the hardcoded regex. This is a SARIF/host-side fix, deliberately separate from `_paths.py::strip_workspace_prefix()` above (that helper serves native-analyzer subprocess output, a different code path with different path shapes).
- **Follow-up review of the two fixes above (issue #846's own PR) caught two of them not actually working, plus a new regression, before merge.** (1) `_paths.py::strip_workspace_prefix()` treated `GITHUB_WORKSPACE` as *exclusive* once set, not preferred-with-fallback — a mismatch (e.g. a self-hosted runner or an `actions/checkout` `path:` layout where `GITHUB_WORKSPACE` and the analyzer subprocess's actual cwd diverge) silently disabled stripping entirely, which is exactly the bug class this issue exists to fix, now reproduced by CI itself. It now tries `GITHUB_WORKSPACE`, then falls back to cwd on a mismatch, genuinely restoring the old guarantee while adding the new one. (2) `analyzers/sarif.py`'s `GITHUB_WORKSPACE`-aware branch is inert under this action's own `container-action` deployment mode: the container forces `GITHUB_WORKSPACE=/workspace`, unrelated to the host path baked into a SARIF file produced by the separate, non-containerized `sarif-prep` job — so the self-hosted-runner scenario the fix claimed to cover was, in fact, still unfixed there. Added `AI_SARIF_HOST_WORKSPACE`, an optional env var a container-mode caller can set to the real host checkout root, checked before `GITHUB_WORKSPACE`. (3) The same SARIF fix applied workspace-prefix stripping to *any* path regardless of whether the original URI was absolute, so a genuinely relative SARIF path starting with a workspace-shaped segment (e.g. `workspace/src/x.py`, plausible in Cargo/pnpm/Bazel-style monorepos) got silently truncated — a new bug this PR itself introduced. Stripping is now gated on the URI having actually been absolute. A SARIF path that still can't be resolved to repo-relative after all three strip attempts now stays absolute (leading slash restored) rather than being handed back as a misleadingly relative-looking string, so `findings/scope.py`'s absolute-path tripwire (added by the third item below) can still catch it — closing a gap where the two halves of this PR didn't otherwise compose. `golangci_lint.py`, `docs_refs.py`, and `semgrep.py` still assume `cwd == repo root` via their own separate (non-stripping) logic and were not folded into `_paths.py` in this pass — known-remaining, noted in `CONTRIBUTING.md`.
- **No pipeline-wide detection when an analyzer emits an absolute `Finding.file` (issue #846)**. `findings/scope.py`'s `apply_diff_scope` silently demoted such a finding to Low/`out_of_diff` (or dropped it entirely, under `AI_ANALYZER_DIFF_SCOPE=drop`) with no signal beyond that — this is exactly how #713 hid until a human noticed it in a dogfood review. Added a warning log (`Finding.file` starting with `/`), aggregated per analyzer source with an occurrence count rather than once per finding (a single misbehaving analyzer can emit hundreds on one file), and worded to match the actual outcome for the active mode.

### Changed

- **Consolidated `eslint.py`/`phpcs.py`/`phpstan.py`'s own cwd-only path-stripping copies onto `_paths.py::strip_workspace_prefix()` (issue #846)**. Each independently re-implemented the same "strip a resolved-absolute prefix back to repo-relative" logic as `ruff.py`/`docs_comments.py` (issue #713), using only `os.getcwd()`/`Path.cwd()` — no `GITHUB_WORKSPACE` awareness, so any of the three could emit a wrong path on a self-hosted runner whose checkout root isn't the analyzer subprocess's cwd. `GITHUB_WORKSPACE` is tried first when set, falling back to cwd on a mismatch or when unset (see the fix entry above for why "GITHUB_WORKSPACE-exclusive" was the wrong first cut).
