...
- **Pre-flight LLM cost estimate and ceiling (issue #24)**. Before any agent dispatches, the review's total LLM spend is estimated from the diff size, the selected agent roster (including the separately-dispatched `pr-summarizer`/`issue-linker` preflight agents, when they will run), and per-model rates in `config/model-pricing.json`, and logged as a `COST_ESTIMATE` line on every run regardless of whether a ceiling is configured. Two new inputs: `max-cost-usd`/`AI_MAX_COST_USD` (default `0`, disabled) sets the ceiling — if the pre-flight estimate exceeds it, the run aborts before any LLM call is made (no dispatch, no partial spend), posting a skip comment via the same mechanism `max-diff-lines` already uses, and exiting `0` by default; `fail-on-cost-ceiling`/`AI_FAIL_ON_COST_CEILING` (default `false`) makes an exceeded ceiling exit `2` instead, mirroring `fail-on-findings`'s own opt-in exit code, for repos that want it as a required CI gate. This PR shipped with unit tests only and no CHANGELOG entry of its own; landed here as part of the #848 follow-up doc-debt pass below, alongside the estimation-bias and other disclosures that pass added on top of it. See [Configuration: Cost ceiling](docs/configuration.md#cost-ceiling) for the full estimation methodology and its documented imprecision.
...
- **Pre-flight cost-ceiling follow-up debt (issue #848, from #845/#24's review)**. Twelve of the review's thirteen tracked items, addressed together since most touch the same small module (the count previously read "Ten" here despite the list below always having had twelve bullets, and the PR description separately said "12 of the 13" -- fixed to agree):
  - **Undisclosed estimation bias** now documented explicitly in `review/cost_ceiling.py`'s module docstring: the estimate is structurally biased high (often 5-10x), pricing every agent's output at its full cap, never discounting prompt-cache reads, and omitting the judge pass's own LLM call entirely.
  - **Duplicated preflight-gating/4096-cap logic** between `review/runtime.py`'s cost estimate and the real dispatch gate in `cli.py`/`review/preflight.py` is now one shared source of truth: `preflight.py`'s new `should_run_pr_summarizer()`/`should_run_issue_linker()` and `PREFLIGHT_AGENT_MAX_TOKENS` constant, used by both call sites.
  - **Duplicated pricing math** between `cost_ceiling.py` and `pricing.py` is now one shared `pricing.compute_cost_units()` helper (sums `(tokens, rate)` pairs before a single floor-division — not two separate divisions summed, which would round differently); `pricing._row_cost` and both `cost_ceiling.py` estimate functions now call it.
  - **Telemetry reported exit 0 for an exit-2 (`AI_FAIL_ON_COST_CEILING`) run.** `TelemetryEvent` gained an `exit_code` field (schema v4); both the skip path and the normal review path in `cli.py` now compute the real exit code once, before emitting telemetry, and pass it through, so the emitted event always agrees with what the process actually exits with.
  - **`action.yml` vs `container-action/action.yml` inconsistent handling for `AI_FAIL_ON_COST_CEILING`**: the container action's `docker run` passthrough forwarded it unconditionally while its sibling `AI_FAIL_ON_FINDINGS` used a `${VAR:+-e VAR}` conditional guard; now consistent.
  - **An unpriced agent could appear as an unflagged "$0" top contributor** in `enforce_cost_ceiling`'s raised message; it now reads e.g. `mystery-agent ($0.0000, unpriced -- no cost data, not necessarily free)`, matching `log_cost_estimate`'s existing `(unpriced)` marker.
  - **No estimate-vs-actual reconciliation**: `review/reporting.py`'s new `log_cost_reconciliation()` logs the pre-flight estimate next to the run's actual total cost once a review completes (`ReviewRuntime.cost_estimate_usd` carries the estimate through) — data collection only, no feedback loop.
  - **Cost-ceiling skip suppressed the free static analyzers.** `review/runtime.py` now runs native analyzers and loads SARIF findings *before* the cost-ceiling check, so a ceiling-exceeded skip (which only stops the LLM-agent roster and the pr-summarizer/issue-linker preflight calls) no longer discards that non-LLM signal; their count is folded into the posted skip-comment reason and logged. Actually posting those findings to the PR during the skip itself would require changing `orchestrate.run_review()`'s `skip_reason` contract — a larger, riskier change left as a follow-up rather than forced through here.
  - **CJK/dense-content undercounting** in `context/budget.py`'s 4-chars-per-token heuristic is now documented explicitly (the heuristic itself is unchanged — a full CJK-aware tokenizer is out of scope).
  - **`cost_ceiling.py` documented** in `CLAUDE.md`'s module table and a new "Pre-flight cost estimate and ceiling" subsection in `docs/architecture-internals.md`.
  - **Skip-comment remediation text addressed the PR author**, who typically cannot set the named env vars (repo/workflow-level config). Reworded to address a repo maintainer, keeping the env var names for reference.
  - **Awkward `merge_cost_estimates` API**: it now accepts bare `AgentCostEstimate` values directly (in addition to `CostEstimate`), removing the throwaway single-entry `CostEstimate` wrapping `review/runtime.py` previously built by hand for each preflight agent.
  - Also strengthened the CLI exit-code test suite for this feature (`tests/python/test_cli.py::TestCostCeilingSkipExitCode`), which previously reimplemented cli.py's exit-code conditional inline and asserted it against itself; it now drives `_run_review_async` end-to-end against a real, tiny `AI_MAX_COST_USD` and asserts the real exit code and posted skip comment.
  - **Deferred, not fixed in this pass**: fully posting analyzer/SARIF findings to the PR during a cost-ceiling skip (see above — needs a larger `orchestrate.run_review()` change).
  - **Second follow-up pass (this PR, same #848 issue)** — an independent comprehensive review of the above surfaced several more real gaps, fixed together here:
    - **`TestCostCeilingSkipExitCode`'s `test_ceiling_not_exceeded_runs_normally_end_to_end` could make real, billable LLM calls if `ANTHROPIC_API_KEY` happened to be set in the ambient environment.** It relied on a *missing* key raising `LLMError`→`SystemExit(1)` as proof the skip branch wasn't taken, with nothing scrubbing the var. Every test in the class now scrubs `ANTHROPIC_API_KEY` via `monkeypatch.delenv(...)`, as defense in depth.
    - **`enforce_cost_ceiling` failed open on unpriced models.** `CostEstimate.any_unknown_pricing` was computed and logged by `log_cost_estimate`, but never consulted at the actual enforcement decision — an agent on an unpriced model (e.g. any `openai-compatible` deployment) contributed `$0`, so the ceiling could never see its real spend, and a run that "passed" was indistinguishable in the log from one that genuinely came in under budget. `enforce_cost_ceiling` now emits a distinct `COST_CEILING_GAP` warning whenever a ceiling is configured and any agent is unpriced, whether or not the priced portion ends up exceeding it. Deliberately does not fail closed (an unpriced model may legitimately be a free/internal one) — only the gap is made loud.
    - **`log_cost_reconciliation` compared two structurally incompatible sets of LLM calls without disclosing it.** The estimate side includes `pr-summarizer`/`issue-linker` but excludes the judge pass; the actual side is the exact opposite (the two preflight agents have no token-usage plumbing at all; the judge pass's real cost is folded into `actual`). The logged `delta=` was biased in both directions at once, undisclosed. Both exclusions are now stated explicitly in the docstring and in the `COST_RECONCILIATION` log line itself. A deeper fix (plumbing preflight-agent token usage, or a judge-pass estimate placeholder) is out of scope here.
    - **Two real regression-guarding tests were deleted, without replacement, by this issue's own test rewrite**: a non-cost-ceiling skip (e.g. `max-diff-lines`) must not become exit 2 even when `AI_FAIL_ON_COST_CEILING` is set (guards against the flag leaking across skip kinds), and the skip-path `result.ok is False` → exit 1 branch. Both restored: the first end-to-end (a real "no changed files" skip against a huge ceiling), the second as a focused unit test of the exit-code formula, since `ReviewResult.ok` returns `True` unconditionally whenever `result.skipped` is set — a real skip can never actually produce `ok=False` for the end-to-end style to exercise.
    - **No test verified a real cost-ceiling skip's `exit_code` reaches the emitted telemetry event end-to-end.** Confirmed the gap was real: reverting just the `exit_code=exit_code` argument at cli.py's SkipPlan-branch `_emit_telemetry(...)` call still passed the full suite. Added `test_ceiling_exceeded_telemetry_event_carries_exit_code_2_end_to_end`, which drives a real ceiling-exceeded skip with telemetry enabled and reads the emitted JSONL event.
    - **The public skip comment disclosed a raw, pre-suppression/pre-diff-scope analyzer+SARIF finding count**, including findings a maintainer had explicitly suppressed or that were out of diff scope and would never surface — plus a false promise ("a future run ... will include them") that doesn't hold once suppression/dedup/scoping are considered, or even for a byte-identical rerun (the estimate is deterministic). The count is now logged only, never appended to the posted skip reason.
    - **The static-analyzer step's `except ImportError: raise` became newly reachable on every run** once analyzers/SARIF were reordered ahead of the cost-ceiling check (the change directly above) — a run that would previously have short-circuited via the ceiling check before ever reaching this code now always reaches it, so a missing/broken analyzer dependency would hard-crash the whole review instead of just failing that one signal soft. Now caught and logged like every other analyzer failure, matching `context/treesitter.py`'s and `docs_comments.py`'s convention for a missing optional dependency, restoring the safety property without reverting the reordering itself.
    - **The 5-10x estimation-bias disclosure only reached contributor-facing docs** (the module docstring, `docs/architecture-internals.md`), not the operator-facing surfaces a maintainer actually configuring `max-cost-usd` would read: `docs/configuration.md`'s Cost ceiling section, and the `max-cost-usd` input description in both `action.yml` and `container-action/action.yml`. All three now carry the figure, the no-caching-discount note, and the no-judge-pass-cost note.
    - **Telemetry schema v4 was undocumented.** `docs/configuration.md`'s telemetry section still said `schema version "2"` and stopped at the v2 field list, missing both v3's `thinking_tokens`/`stop_reason` (#592) and v4's `exit_code` (this issue). Updated to describe the current schema and full field list.
    - **`TelemetryEvent`'s own docstring first line still said "schema version 3"** despite the schema being bumped to 4 two lines below it, in the same docstring. One-line fix.
    - **A stale line-number comment in `TestFailOnFindings`** (`"lines 271-275"`, from before this PR moved that logic) now points at the exit-code decision's current location and cross-references `TestCostCeilingSkipExitCode`'s end-to-end equivalent.
    - **`_emit_telemetry`'s docstring claim that "every call site computes its real exit code before calling this"** was false for the dry-run call site, which relied on the dataclass default of `0` rather than passing it explicitly. Now passes `exit_code=0` explicitly there, matching every other call site.
    - **The `CHANGELOG.md` entry above said "Ten of the review's thirteen tracked items" while listing twelve** (and the PR description separately said "12 of the 13"). Fixed to agree (twelve).
    - **Issue #24's underlying cost-ceiling feature itself had shipped with zero `CHANGELOG.md` entry** — only this issue's own `### Fixed` follow-up entry ever mentioned cost-ceiling, so release notes would show a fix for a feature nobody was told shipped. A proper `### Added` entry for #24 is now above.
