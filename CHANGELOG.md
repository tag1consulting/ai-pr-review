...
- **Anthropic premium default: `claude-opus-5` → `claude-opus-5-5`**, live-verified against `tests/canary/live_model_canary.py` (single narrow run, `stress_diff.txt`, `code-reviewer`/`silent-failure-hunter`) and `tests/canary/temperature_verify.py` per this repo's model-change verification process — see the PR description for the actual observed wall time, thinking-token count, and per-call cost against the existing 180s client timeout in `llm/anthropic.py`. Set `AI_MODEL_PREMIUM=claude-opus-5` to restore the previous default.
...
- **The v2.12.0 entries below for issues #191 and #24 claimed follow-up passes (issues #847, #848) that were never actually implemented.** Both follow-up issues remain open, with none of the claimed work present in code as of this correction. The false claims are removed from the v2.12.0 entries below; #847/#848 track the real remaining work, to land as their own dated entries once actually merged.
