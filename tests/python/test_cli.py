...
class TestWriteStepSummary:
    ...
    def test_failed_summary_post_dumps_findings_to_step_summary(
        ...
        Mirrors production: orchestrate.py always synthesizes a real
        FindingsResult with a "skipped: summary post failed (...)" error
        whenever post_summary fails (never leaves findings_post as an
        untouched truthy mock), so both error lines render together.
        """
        from ai_pr_review.findings.models import Finding
        from ai_pr_review.review.reporting import write_step_summary as _write_step_summary
        ...
        assert "Summary post error:" in content
        assert "HTTP 401: Bad credentials" in content
        assert "Findings post error:" in content
        assert "skipped: summary post failed" in content
    ...
    def test_failed_findings_post_dumps_findings_to_step_summary(
        ...
        assert "HTTP 401: Bad credentials" in content
        assert "SQL injection in login handler" in content
        assert "Summary post error:" not in content

    def test_failed_post_with_no_findings_still_shows_banner(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """A failed post_summary with zero computed findings still renders the
        banner and error line, but no finding list (#588)."""
        from ai_pr_review.review.reporting import write_step_summary as _write_step_summary
        from ai_pr_review.vcs.protocol import FindingsResult, SummaryResult
        ...
        result.findings_post = FindingsResult(
            review_id=None, inline_posted=0, body_findings=0, event="COMMENT",
            error="skipped: summary post failed (HTTP 401: Bad credentials)",
        )
        result.findings = []
    ...
    def test_run_review_async_writes_step_summary_before_failure_exit(
        ...
        """_run_review_async must write the step summary even when result.ok
        is False (#588). This drives the real call site in cli.py rather than
        calling write_step_summary directly, so it fails if the call is ever
        moved after the `if not result.ok: return 1` early return or wrapped
        in a `result.ok` guard.
        """
        import anyio
        ...
        assert exit_code == 1, "result.ok=False must still exit 1"
        content = summary_path.read_text()
        assert "⚠️ Posting failed" in content
        assert "SQL injection in login handler" in content
