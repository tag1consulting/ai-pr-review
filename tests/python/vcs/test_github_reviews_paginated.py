...
def test_list_bot_reviews_keeps_pending_and_returns_full_review_dicts() -> None:
    ...
    # and ai_pr_review.slash.github_ops read "user"/"state"/"body" off of it.
...
def test_both_walks_agree_skipping_an_empty_body_reply_created_review() -> None:
    """Same consolidation guarantee as above, but with a higher-id review
    whose body is empty -- the shape of GitHub's auto-created review from a
    bot reply via the REST replies endpoint. Both walks must still agree,
    and must agree on review 3, not the empty-bodied review 4."""
    reviews = [
        _review(1, "COMMENTED", body="oldest"),
        _review(3, "CHANGES_REQUESTED", body="newest, generated by [ai-pr-review]"),
        _review(2, "DISMISSED", body="middle"),
        _review(4, "COMMENTED", body=""),
    ]
