...
Check the CI job log's `Token usage by agent` breakdown (always echoed there, on every provider — see [Features: Token usage](features#token-usage)), not the posted review comment: by default the comment carries only a compact summary line with an agent *count*, not names, so it can't tell you *which* agents ran. The job log's full table names every agent that actually ran — no `Token usage by agent` table at all (rather than an empty one) on a route configured with `agents: []` confirms it matched and suppressed the roster as expected. On GitHub, the `GITHUB_STEP_SUMMARY` page works the same way. Prefer checking a route's *first* automatic review on a fresh PR when verifying: repeatedly re-triggering `/ai-pr-review rescan` on the same PR can interact with incremental-diff caching in ways that make a single anomalous result hard to interpret in isolation.
