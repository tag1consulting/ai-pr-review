...
| `AI_BITBUCKET_CODE_INSIGHTS` | `true` | Bitbucket only. Renders eligible findings as a [Code Insights](https://support.atlassian.com/bitbucket-cloud/docs/code-insights/) report with diff-anchored annotations, rebuilt from scratch (delete, then recreate) every run — the closest Bitbucket equivalent to GitHub/GitLab inline comments (issue #839, closed by the same work as issue #873). Also gates cross-run dedup: a dismissed finding is excluded from the next rebuild as long as it stays at the same file/line (exact-fingerprint match only, no fuzzy match). Falls back to rendering every finding flat in the comment body if Code Insights isn't available on your workspace's plan (a 403/404 on the report API) — nothing is silently dropped. See [Bitbucket setup](bitbucket-setup#code-insights-annotation-category-mapping). |
| `AI_BITBUCKET_VERDICTS` | `false` | Bitbucket only, opt-in. Bitbucket Pipelines has no comment-triggered event, so `/ai-pr-review dismiss\|false-positive\|wont-fix\|fixed F<n>` commands posted as a fresh top-level PR comment are picked up and applied at the start of the *next* review run instead of immediately (issue #874). Body-level only — Bitbucket findings anchor via Code Insights annotations, which aren't repliable, so there is no inline-reply form. An unset/empty/unrecognized value resolves to disabled rather than silently enabling a security-relevant feature. |
| `AI_BITBUCKET_VERDICT_MIN_ROLE` | `write` | Bitbucket only, only checked when `AI_BITBUCKET_VERDICTS=true`. Minimum PR permission (`read`/`write`/`admin`) a commenter must hold for their verdict command to be accepted. Checked fail-closed: a permission-lookup error rejects the command rather than falling back to accepting it, since a false accept here could let any commenter silence a real security finding. An unrecognized value raises at startup rather than silently falling back to a weaker role. |
