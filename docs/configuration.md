...
`AI_MAX_TOKENS_CODE_REVIEWER`). Invalid values are handled two ways, not one
(#847): a non-integer value (e.g. `abc`) prints a warning and falls back to
the pre-override value, same as `AI_MAX_TOKENS_PER_AGENT`; a value outside
256–65536 prints a warning and is **clamped** to whichever bound it crossed
(256 or 65536) — that clamped value is used, not the pre-override value. Both
kinds of invalid input are warned rather than failing the run. Every set
`AI_MAX_TOKENS_<AGENT>` override is validated at startup regardless of
whether that agent actually runs this review, so a typo'd value is never a
silent no-op. See [issue #191](https://github.com/tag1consulting/ai-pr-review/issues/191).
