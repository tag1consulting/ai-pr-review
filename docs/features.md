...
This full canonical-review-reuse behavior is GitHub-only. GitLab has a narrower analog: `post_findings` fuzzy-matches each finding against this bot's still-open prior discussions (same file, within 3 lines, compatible category) and skips reposting an unchanged one, keeping the existing discussion open instead — but GitLab has no dismiss/false-positive/wont-fix/fixed verdict system, so there's no suppression, no escalation reply, and no in-place update of an existing discussion's body yet; a `resolve_stale` run also never re-resolves a discussion this same run just created or matched. Set `AI_GITLAB_CROSS_RUN_DEDUP=false` to restore GitLab's pre-dedup behavior of posting a fresh discussion for every eligible finding on every run. Bitbucket still posts fresh review content on every run (tracked as a parity gap, issue #710). Set `AI_CANONICAL_REUSE=false` to disable GitHub's reuse entirely and restore its pre-reuse behavior of always posting a fresh review — see [Configuration](configuration#quiet-reruns-and-cross-run-finding-dedup).
