...
**Canonical review**: `select_canonical(reviews)` — the highest-`id` bot review with a non-empty body among `GitHubProvider._list_prior_bot_reviews()`'s output, regardless of state. Reviews with an empty or whitespace-only body are skipped: GitHub auto-creates one of these (state `COMMENTED`) every time the bot replies to an inline comment via the REST replies endpoint (the severity-escalation notice, a recurred-finding reply, a feedback acknowledgement), and GitHub rejects any attempt to `PUT` a body onto such a review with HTTP 422. `ai_pr_review.slash.github_ops._record_verdict` (the write side) calls this same function directly rather than reimplementing the rule, so both sides of the split are structurally guaranteed to agree on which review is canonical *given the same review list*. `_record_verdict` itself is fed from the separate `list_bot_reviews()` method, which keeps a looser contract than `_list_prior_bot_reviews()` (partial results on a mid-pagination error rather than `[]`, and no `PENDING`-state filter) — both methods now share one paginated walk (`GitHubProvider._list_reviews_paginated(*, strict, states)`), so they can no longer *independently* drift on pagination or error-handling bugs, but the two contracts remain deliberately different: `list_bot_reviews()`'s other callers (`ai_pr_review.slash.github_ops`'s F-id classification and the PR-wide auto-approve check) need every review regardless of state and tolerate a partial fetch, so unifying the contract there would cost real functionality for a discrepancy window that, in practice, never opens — the bot always submits its own reviews with an explicit `event`, so it never leaves one in `PENDING`. A review in `PENDING` state or a fetch error affecting only one side can therefore still, in principle, make the two sides disagree about which review is canonical; this is an accepted, documented tradeoff, not a bug this consolidation set out to close.
...
- `id_map` (the fingerprint to `F<n>` map) is assembled once, before classification runs, so a reworded finding under `update`/`escalate` classification got a *second*, freshly minted `F<n>` id that was baked into the review body's id-map marker but never rendered anywhere: a wasted id, and a trap for `fingerprint_for_finding_id`'s (`ai_pr_review/vcs/_finding_ids.py`, called by `ai_pr_review.slash.github_ops` since #849) reverse F-id-to-fingerprint lookup, which would resolve the visible token to the wrong (or no) fingerprint.
...
**Posting the skip-time findings (#896).** Those pre-computed analyzer/SARIF findings are no longer discarded: the `SkipPlan` also carries them (`extra_findings`), plus the real diff, suppression rules, and diff-scope mode, so `orchestrate.run_review()`'s skip branch can run the same suppress → diff-scope → rollup pipeline a normal run uses (in-process, no LLM call — agent dispatch and the judge pass are both skipped entirely) and render the kept findings as a body-level list inside the one skip comment (`post_skip_comment(reason, findings=kept)`). This deliberately bypasses `post_findings`: no F-IDs are assigned (no slash command can target one of these findings individually), no incremental-watermark advance, no stale-thread resolution, and no formal `APPROVE`/`REQUEST_CHANGES` event — a skip's blast radius stays a single comment. `AI_FAIL_ON_FINDINGS` still applies: it reads the skip's own outcome classification (`ReviewOutcome.may_approve`) exactly the way the non-skip path's exit code does, independently of `AI_FAIL_ON_COST_CEILING` — a clean cost-ceiling skip stays exit 0 even with `fail-on-findings` set, and a skip whose surviving findings would block approval exits 2 even if `fail-on-cost-ceiling` is unset. `AI_DRY_RUN` is honored on this path too, suppressing the post entirely.
...
The classifier's result is then passed through `cap_review_outcome()` (issue #858) before it reaches a VCS provider — both production call sites in `orchestrate.py` do this. `AI_APPROVAL_CEILING` (default `approve`, a no-op) clamps `event` to at most `REQUEST_CHANGES` or `COMMENT`, but never touches `may_approve`/`risk`/`incomplete`/`finding_total`: `cli.py`'s `AI_FAIL_ON_FINDINGS` exit-code check reads `may_approve`, not `event`, specifically so a configured ceiling changes what gets *posted* without changing the CI exit code. See [Configuration: Approval ceiling](configuration.md#approval-ceiling).
