...
**Canonical review**: `select_canonical(reviews)` — the highest-`id` bot review among `GitHubProvider._list_prior_bot_reviews()`'s output, regardless of state. `ai_pr_review.slash.dismiss._record_verdict` (the write side) calls this same function directly rather than reimplementing the rule, so both sides of the split are structurally guaranteed to agree on which review carries the markers — though `_record_verdict` itself is fed from the separate `list_bot_reviews()` method, which has looser error-handling semantics (partial results on a mid-pagination error, no PENDING-state filter) than `_list_prior_bot_reviews()`'s all-or-nothing/four-state-filtered contract; a review in `PENDING` state or a fetch error can in principle make the two sides disagree (tracked as a follow-up, not yet consolidated into one shared walk).
...
Thread ownership (`parse_prior_thread`, called with `bot_login=None`) is gated solely by the per-comment marker, not by comparing GitHub's GraphQL-reported author login against the REST-style `"<login>[bot]"` constant — GraphQL reports the bot's login without the `[bot]` suffix, so that comparison would reject every real thread (verified live; see `ai_pr_review.slash.dismiss`'s established use of the same `bot_login=None` pattern for GraphQL-sourced authors). `resolve_stale`/`_dismiss_stale_reviews` do not yet follow this pattern — tracked as a separate, pre-existing issue independent of this feature.
