...
`pricing.compute_cost_units()` is the shared `(token_count, rate)`-pairs-summed-then-divided-once formula behind both `_row_cost` (actual usage, above) and `review/cost_ceiling.py`'s pre-flight estimate (below) — consolidated in #848 so the two can't independently drift; summing pairs before one floor division (rather than floor-dividing each pair separately and summing) matters because integer floor division does not distribute over addition.

### Pre-flight cost estimate and ceiling (#24 / #848)

`ai_pr_review/review/cost_ceiling.py` estimates a review's total LLM spend *before* any agent dispatches, and can skip the run entirely if the estimate exceeds a configured ceiling:

- `estimate_review_cost()` prices the main agent roster: input tokens from the diff plus (for context-enrichment-eligible agents) the shared PR-context block and language-profile text, via `context/budget.py`'s `estimate_tokens()` 4-chars-per-token heuristic; output tokens at the agent's *effective cap* (`AI_MAX_TOKENS_PER_AGENT` or the agent's own `max_output_tokens`) — an upper bound, not a prediction.
- `estimate_preflight_agent_cost()` separately estimates the two separately-dispatched preflight agents (pr-summarizer, issue-linker), using diff size alone as a coarse input proxy and the shared `PREFLIGHT_AGENT_MAX_TOKENS` (4096) constant for output — `review/preflight.py`'s `should_run_pr_summarizer()`/`should_run_issue_linker()` gate whether each is folded in, mirroring cli.py's real dispatch gate exactly (#848 consolidated what used to be two independently-maintained copies of this condition).
- `merge_cost_estimates()` combines the main roster's `CostEstimate` with individual `AgentCostEstimate`s into one aggregate, recomputing totals from the merged per-agent list.
- `log_cost_estimate()` always emits a `COST_ESTIMATE` WARNING line (whether or not a ceiling is configured) so the estimate is auditable in every run's logs; unpriced agents are marked `(unpriced)` in the breakdown.
- `enforce_cost_ceiling()` raises `CostCeilingExceeded` when the total exceeds `AI_MAX_COST_USD`; `review/runtime.py` catches this and returns a `SkipPlan` (`is_cost_ceiling_skip=True`) before any LLM call — by default this is an informational skip (exit 0), and only becomes a CI failure (exit 2) when `AI_FAIL_ON_COST_CEILING` is set. The raised message names a repo maintainer (not the PR author) as the audience for the remediation knobs, since they are repo/workflow-level settings a PR author cannot change from the PR itself.
- **Known estimation bias** (disclosed in the module's own docstring): the estimate is structurally biased *high*, often 5-10x, because it prices every agent's output at its full cap, does not discount prompt-cache reads at all, and omits the judge pass's own LLM call entirely. Treat `AI_MAX_COST_USD` as "no more than N times a bad-case run," not "no more than $N."
- **Static analyzers/SARIF still run on a cost-ceiling skip** (#848): `review/runtime.py` runs native analyzers and loads SARIF findings *before* the cost-ceiling check, so a ceiling-exceeded skip (which only stops the LLM-agent roster and the pr-summarizer/issue-linker preflight calls) doesn't also suppress that free, non-LLM signal. Their finding count is folded into the posted skip-comment reason and logged, though actually posting those findings to the PR during a cost-ceiling skip is a larger change (it would need to alter `orchestrate.run_review()`'s `skip_reason` short-circuit) left as a follow-up.
- **Estimate-vs-actual reconciliation**: `review/reporting.py`'s `log_cost_reconciliation()` logs the pre-flight estimate next to the run's actual total cost (from `pricing.compute_totals()`) once the review completes, purely for comparison in the logs — data collection only, no feedback loop or automatic recalibration.
