...
- Cross-run finding dedup: an unchanged finding fuzzy-matches (same file, within 3 lines, compatible category) this bot's still-open prior discussions and isn't reposted every run, keeping the existing discussion open (issue #710). A changed finding gets the matched discussion's note updated in place instead of a fresh post, and an escalating severity also gets a reply noting the change. Narrower than GitHub's canonical-review reuse — no dismiss/false-positive/wont-fix/fixed verdicts on GitLab, so there's still no suppression. Set `AI_GITLAB_CROSS_RUN_DEDUP=false` to disable.
