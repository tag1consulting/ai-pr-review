...
- **2.FR-5** `ai_pr_review/agents/summarizer.py` ports `pr-summarizer` with Mermaid sequence-diagram generation rebuilt around typed function-call traces (resolves #100). _(Superseded: sequence-diagram generation was later removed; the summarizer now emits summary, PR type, effort, and the walkthrough table only.)_
