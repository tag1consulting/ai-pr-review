...
- [ ] **Task 1 — `prompts/finding-judge.md`: Judge prompt** (AC: 6)
  ...
  - [ ] Output contract section in the prompt:
    ```json
    {"verdicts": [{"id": <int>, "verdict": "keep"|"downrank", "reason": "<one line>"}]}
    ```
  - [ ] Include an empty-state instruction: if all findings should be kept, return all with `keep`.

- [ ] **Task 2 — `ai_pr_review/findings/judge.py`: Core module** (AC: 1-5)
  - [ ] `JUDGE_DOWNRANK_AMOUNT: int = 15`
  - [ ] `JudgeVerdict = Literal["keep", "downrank"]`
  - [ ] `async def judge_findings(kept: list[Finding], *, llm_call: LLMCall, model: str, prompt_path: Path) -> list[Finding]`
  ...
  - [ ] `def _apply_verdicts(kept: list[Finding], verdicts: list[dict[str, object]]) -> tuple[list[Finding], int]`
  ...
  - [ ] Add `from __future__ import annotations`; full type annotations; module docstring

- [ ] **Task 3 — `ai_pr_review/config.py`: Config toggle** (AC: 7)
  - [ ] Add `AI_JUDGE_PASS` to `_KNOWN_AI_VARS` frozenset
  - [ ] Add `enable_judge_pass: bool = True` to `ReviewConfig` (near `enable_feedback_loop`)
  - [ ] Add `enable_judge_pass=_bool("AI_JUDGE_PASS", True)` in `from_env`

- [ ] **Task 4 — `ai_pr_review/review/runtime.py`: Thread into `OrchestrationConfig`** (AC: 7, 8)
  - [ ] Add `enable_judge_pass: bool` to `OrchestrationConfig` (or equivalent — check how `enable_context_enrichment` is threaded at lines 311-320 and mirror exactly)
  - [ ] Set it from `cfg.enable_judge_pass`
  - [ ] Thread `prompt_path` for the judge prompt (`script_dir / "prompts" / "finding-judge.md"`) if `OrchestrationConfig` carries prompt paths; otherwise pass it in the orchestrate call

- [ ] **Task 5 — `ai_pr_review/orchestrate.py`: Phase 2.75** (AC: 8)
  - [ ] Add import: `from ai_pr_review.findings.judge import judge_findings`
  - [ ] After the `rollup_repeated_findings` block (line ~200, end of Phase 2.5) and before `classify_review_outcome` (line ~206):
    ```python
    ...
    ```
  - [ ] Log the downrank count at INFO level after the call

- [ ] **Task 6 — Tests** (AC: 9, 10)
  - [ ] **New `tests/python/test_judge.py`:**
  ...
  - [ ] **Extend `tests/python/test_orchestrate.py`:**
...
- [ ] **Task 7 — Verification** (AC: 10)
  - [ ] `pytest tests/python -q` — full suite green
  - [ ] `mypy ai_pr_review/` — clean (83+ source files)
  - [ ] `ruff check ai_pr_review/ tests/python/` — clean
  - [ ] `python -c "import ai_pr_review.findings.judge; import ai_pr_review.orchestrate"` — no import error
