...
PR: https://github.com/tag1consulting/ai-pr-review/pull/556 (merged at 7adb0752, 2026-07-02)
...
- **Post-approval:** the `GH_TOKEN` PAT on `ai-pr-review-test` (the token blocking the `bot_login=None` live-verification item above) was regenerated and live-verified in a separate task; see memory `project_github_pat_needs_fix.md`. Not re-opened in this story since the `bot_login` design question itself is still deferred to 13-2's live-e2e smoke.
- **Review findings addressed (commit b895705):** the ai-pr-review bot flagged 3 findings on PR #556 (2 Medium inline, 1 Low body-level), all confirmed real and fixed before merge:
  - F1/F2: `dismiss_by_finding_id`/`dismiss_inline_reply` always returned a "resolved the thread" reply even when `resolve_thread` failed (`resolved=False`), misleading the caller with a success-sounding message alongside populated `errors`. Both now branch on `resolved` and report the failure explicitly.
  - F3: `dismiss_by_finding_id`'s "could not find finding" reply fired even when the lookup itself failed (partial results from `list_bot_reviews` after a mid-pagination HTTP error). Fixed at the reply layer — `list_bot_reviews`'s partial-results-on-error behavior was deliberately left unchanged (an empty-list return would not have fixed the underlying issue, since `classify_finding([], n)` also returns UNKNOWN; the caller must check `provider._errors` regardless).
  - 2 new regression tests added; full suite at 1708 tests (up from 1706), mypy/ruff clean.
...
- `tests/python/vcs/test_dismiss_github.py` — new file: 11 tests covering BODY/INLINE/UNKNOWN orchestration outcomes, same-call resolve+dismiss, the #555 error-surfacing class for both orchestration functions, a locking test for the `bot_login=None` design choice, and honest-reply regressions for the resolve-fails and lookup-fails paths (review findings F1/F2/F3)
