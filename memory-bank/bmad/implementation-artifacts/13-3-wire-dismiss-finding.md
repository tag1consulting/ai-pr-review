...
- [x] Task 4 — Live-e2e smoke, CLI substance (AC: 13, partial — see Debug Log for deferred scope)
  - [x] Built the branch image locally and ran `ai-pr-review dismiss-inline` via `docker run` against the real GitHub API on `tag1consulting/ai-pr-review-test#1`: confirmed provider construction, the ownership gate correctly rejecting a real (pre-`INLINE_MARKER`) bot comment as not-ours (`::notice::reaction=confused`, honest "not posted by this bot" reply), and clean stdout/stderr separation
  - [ ] Happy-path resolve+dismiss round-trip against a fresh `INLINE_MARKER`-bearing thread on a `CHANGES_REQUESTED` review — the test repo currently has no such fixture (its only unresolved threads belong to an already-`DISMISSED` review, and its only `CHANGES_REQUESTED`-eligible reviews have no inline comments, both consequences of an unrelated review-posting anomaly under separate investigation); manufacturing one would require triggering that unrelated pipeline. Deferred to the epic's combined `:dev` pre-tag pass alongside story 13-2's deferred comment-flow scope and issue #562's verification
...
- `ai_pr_review/cli.py` — new `dismiss-inline` subcommand; new `_build_github_provider_or_exit()` helper (also used by `dismiss`, refactored to call it); both `dismiss` and `dismiss-inline` now echo `DismissResult.errors` as `::warning::` lines instead of swallowing them
- `tests/python/test_cli_dismiss_inline.py` — new test file (9 tests)
