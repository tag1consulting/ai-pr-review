...
**PR #443/#445 produced ctx_tok=0 with enrichment ON**: `_detect_primary_language` returned empty for `ai_pr_review/config.py` + `tests/python/test_config.py` files. Likely the Python language key lookup is returning empty string when changed_files contains `.py` files — needs investigation. PR #445 (docs-only: `CHANGELOG.md`, `action.yml`, prompts) correctly produced no context.
...
**Next steps (Phase B from plan, before any flip)**:
1. Add `tests/python/test_context_symbols.py` covering ripgrep no-op, path-confinement, query cap, proximity classification (no tests exist for `symbols.py` today).
2. Investigate why PR #443 showed `ctx_tok=0` despite `.py` files — `_detect_primary_language` may not be seeing the language correctly when `changed_files` is a plain list vs `ChangedFiles` object.
3. If proceeding with container-only flip: update `action.yml` + `container-action/action.yml` defaults independently (container-action → `true`, composite action → keep `false`), or add a note to the composite action input description that enrichment requires the container image.
